...
public class Game {
    ...
    public Game(String playerOneMark, boolean playerOneTurn, UserInput userInput) { 
    	if (playerOneMark == "X") {
    		this.playerOneMark = "X";
    		this.computerMark = "O";
    	}
    	else {
    		this.playerOneMark = "O";
    		this.computerMark = "X";
    	}
    ...
    public void displayGrid() {
    	for (int column = 0; column < 3; column++) {
    		...
    		for (int row = 0; row < 3;row++) {
    			...
    			else if (grid[row][column] == 1) { // playerOne space
    				System.out.print(" " + playerOneMark );
    			}
    			else if (grid[row][column] == 2) { // playerTwo space
    				System.out.print(" " + computerMark);
    			}
    			if (row != 3) {
    				System.out.print("  |");
    			}
    ...
    public void play() {
    	...
    	if (playerOneTurn) {
            ...
            String validCoordinate = userInput.askForCoordinate(grid);
            ...
            x = Integer.parseInt(validCoordinate.split(",")[0]);
            y = Integer.parseInt(validCoordinate.split(",")[1]);
       
         	
            grid[x][y] = 1;
     		openSpaces--;
     		if (checkIfWin('P')) { // check if playerOne won
     			gameIsComplete = true;
     			result = 'P';
     			this.displayGrid();
     		}
    ...
    public boolean checkIfWin(Character c) {
    	int val = c == 'P' ? 1 : 2; // who are we checking to see if won, playerOne or the Computer ?
